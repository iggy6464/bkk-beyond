document.addEventListener('DOMContentLoaded', () => {
  ...
  const translations = {
    ko: {
      ...
      "result.cta": "🗺 비밀 지도 무료로 받기",
      "result.share": "📤 결과 공유하기",
      "result.shareTitle": "나의 방콕지수 결과 | BKK BEYOND",
    ...
    en: {
      ...
      "result.cta": "🗺 Get the Secret Map — Free",
      "result.share": "📤 Share My Result",
      "result.shareTitle": "My BKK Index | BKK BEYOND",
    ...
    th: {
      ...
      "result.cta": "🗺 รับแผนที่ลับ — ฟรี",
      "result.share": "📤 แชร์ผลลัพธ์",
      "result.shareTitle": "ดัชนีกรุงเทพฯ ของฉัน | BKK BEYOND",
  ...
  let currentStep = 0;
  let totalScore = 0;
  let lastResult = { score: 0, percent: 0, level: '' };
  ...
  function showQuizResults() {
    ...
    // 공유·리드캡처가 참조할 결과 컨텍스트 저장
    lastResult.score = totalScore;
    lastResult.percent = Math.round((totalScore / 120) * 100);
    lastResult.level = levelTitle;
  ...
  function showToast(msg) {
    let toast = document.getElementById('bkk-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'bkk-toast';
      toast.className = 'bkk-toast';
  ...
  /* --- 결과 → 뉴스레터 전환 브릿지 --- */
  const btnResultCta = document.getElementById('btn-result-cta');
  if (btnResultCta) {
    btnResultCta.addEventListener('click', () => {
      // 리드에 퀴즈 레벨 태깅 (세그먼테이션용)
      const ctx = document.getElementById('lead-context');
      if (ctx) ctx.value = `quiz:${lastResult.percent}%:${lastResult.level}`;
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener('submit', async (e) => {
      ...
      if (!EMAIL_RE.test(email)) {
        formMsg.innerText = t["newsletter.error"];
        formMsg.className = "form-message error";
        emailEl.focus();
        return;
      }

      // 2) 리드 구성 (퀴즈 레벨 컨텍스트 + 언어 태깅)
      const lead = {
        email,
        context: (document.getElementById('lead-context') || {}).value || 'direct',
        lang: currentLang,
